...
#include <QQuickView>
#include <QtMath>
QGC_LOGGING_CATEGORY(BatteryLog, "BatteryLog")

static const char* kGroupName       = "BatterySettings";
    ...
    : QObject(parent)
{
    ...
    qCDebug(BatteryLog) << "Creating Battery instance";
    QSettings settings;
...
void Battery::setvehicle(Vehicle *set)
{
    if(_vehicle!=set){
        _vehicle=set;
        emit vehicleChanged();
...
void Battery::setbatt(VehicleBatteryFactGroup* set)
{
    if(_batt!=set){
        _batt=set;
        emit battChanged();
...
void Battery::setShowFeatures(bool set){
...
void Battery::setCellNumber(int set){
...
void Battery::setFeatures(int set){
    if (_features!=set ){
        _features=set;
        QSettings settings;
        settings.beginGroup(kGroupName);
...
void Battery::setCellCapacity(int set){
...
uint8_t Battery::level(uint16_t voltage,uint16_t minVoltage, uint16_t maxVoltage,mapFn_t mapFunction) {
